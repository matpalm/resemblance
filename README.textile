...
<pre>
> cat test.data | ./shingle.rb coeff 0.5 > result
</pre>
...
compare simhash to brute force n^2 compare all (considering only values above resemblance 0.5)

<pre>
> cat test.data | ./shingle.rb coeff 0.5 | sort -nr -k3 > shingling.result
> cat test.data | ./simhash.rb | sort -nr -k3 > simhash.result
> ./compare.rb 0.5 shingle.result simhash.result
</pre>

h1. sketching heuristic

compare sketching to brute force n^2 compare all (considering only values above resemblance 0.5)
use 64bit hash, calculate 10 shingles and cutoff at MAX/2

<pre>
> cat test.data | ./shingle.rb coeff 0.5 | sort -nr -k3 > shingling.result
> cat test.data | ./sketch.rb 64 10 2 | sort -nr -k3 > sketch.result
> ./compare.rb 0.5 shingle.result simhash.result
</pre>
